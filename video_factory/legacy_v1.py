"""v1 模板视频生成器（已退役，仅保留入口兼容）。

这是被 5 段流水线（改写 → 配音 → 出图 → 拼装 → 发布）取代的旧路径：写死的世界杯
demo 分镜 + PIL 逐帧绘制 + ffmpeg concat。它只服务 `python -m video_factory`
（cli.py），项目文档零提及，任何在用的流水线代码都不引用它；保留仅为不删掉用户
可能还在用的命令行入口。

依赖方向是单向的 legacy_v1 → pipeline：TTS 引擎留在 pipeline.py，本模块复用它的
provider 实现，并额外提供依赖分段结构的 tone 兜底音轨。绝不能反向依赖，否则
pipeline 会重新拖起 PIL，拖慢每一个 stage 的导入。
"""
...
from video_factory.pipeline import (
    ...
    _synthesize_doubao_voiceover,
    _synthesize_edge_voiceover,
    _synthesize_file_voiceover,
    _synthesize_openai_voiceover,
    _write_voiceover_notes,
)
