...
# 次数随片长动态：每 ~15s 一个理想槽位，封顶 30；实际次数仍由 AI 导演式的候选
# 质量决定——补填只收 6~16 字的适配短句、避让窗（开屏/金句卡）照旧、最小间隔兜底，
# 凑不齐就自然少出。
# 2026-07-23 用户定案密度再降（原每 6s 太密，150s 塞 ~25 个）：每 15s 一个，降幅 ~60%。
SYNC_MOMENT_EVERY_S = 15.0
SYNC_MOMENT_MAX_COUNT = 30
# 相邻时刻最小间隔（秒）：必须 ≥ SYNC_MOMENT_MAX_DURATION，否则后一条会在前一条还没
# 消失时就叠上来——所有中段文案都锚在同一屏幕位置（顶部 28%/30% 居中），时间上一交叠
# 就直接互相遮挡。2026-07-23 用户实测遮挡：原值 4.0 < 特效时长 7.0，最多重叠 3s；
# 提到 7.0（=时长封顶）后保证「上一条消失、下一条才出」，根除同位置文案重叠。
SYNC_MOMENT_MIN_GAP = 7.0
